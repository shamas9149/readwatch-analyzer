...
if count == 0:
   with open("Goodread.csv", mode="r",newline="",encoding="utf-8") as file:
    ...
    for row in csv_reader:
        ...
        sql="INSERT INTO books(Title,author,rating,pages,exclusive_shelf) VALUES (?,?,?,?,?)"
        val=(Title,Author,rating,pages,exclusive_shelf)
        book_cursor.execute(sql,val)
        connection_books.commit()


#Letterbox shifting data to sql
    ...
    while True:
        ...
        if choice == "1":
            ...
            while True:
                ...
                print("7. 5. Movies Watched by Year")
                print("8. Back") 
                ...
                if choice == "1":
                  ...
                  print("Movies Watched: ",Count)
                    ...
                    movies_cursor.execute("""
                        SELECT strftime('%Y-%m', when_watched) AS month, COUNT(*) as cnt
                        FROM movies
                    ...
                    movies_cursor.execute("""
                        SELECT strftime('%Y', when_watched) AS year, COUNT(*) as cnt
                        FROM movies
                        WHERE special_character='Watched'
                        GROUP BY year
                        ORDER BY year
